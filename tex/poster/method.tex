...
\begin{columnfigure}
	\centering
	\input{model.tex}
	\figcaption{Diagram of model. Originals, $\vec{x}\idx{HR}$, are binarised and downsampled, $\vec{x}\idx{LR}$. Reconstructrions, $\tilde{\vec{x}}\idx{HR}$, are thet results of the VAE.}
	\label{fig:diagram}
\end{columnfigure}
...
\begin{itemize}
	...
	\item \textbf{Neural networks} output distribution parameters $\vec{\mu}_\phi(\vec{x})$ and $\vec{\sigma}^2_\phi(\vec{x})$ for $\vec{z}$ and $\vec{\mu}_\theta(\vec{z})$ for $\dec{\vec{x}|\vec{z}}$.
