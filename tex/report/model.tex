...
\begin{tikzpicture}[x = 1cm, y = 1cm, thick,
	...
	\node[image, minimum size = 2 cm] (HR) at (0, 0) {\includegraphics[width = 2cm]{example_original_ds2_l30_e50}};
	\node[below = .5cm of HR] (xHR) {$\vec{x}_{HR}$};
	\node[above = 0.1cm of HR] (xdim_HR) {28};
	\node[left = 0.1cm of HR] (ydim_HR) {28};

	\node[image, minimum size = 1 cm, right = of HR] (LR) {\includegraphics[width= 1cm]{example_downsampled_ds2_l30_e50}};
	...
	\node[image, minimum size = 3 cm, right = of decoder] (reconstruction) {\includegraphics[width= 3cm]{example_reconstructed_ds2_l30_e50}};
